...
+-------------------------------+------------------------+------------------+
|  Warm Concrete Editorial UI   | Drizzle Prepared SQL   | Phase-Based Loop |
|  High Contrast, Silk Glow     | Red-Green-Refactor     | Manifests & Rot  |
+--------------------------------------------------------------------------+
```
...
Para qualquer alteração ou criação de interface de usuário (UI/UX), o desenvolvimento deve ser guiado sob as premissas estéticas de luxo **Monumental Editorial / Warm Concrete**:

### Diretrizes Estéticas Críticas:
* **Paleta de Cores Warm Concrete (JurisAI Premium):**
  * Background: `#f5f5f4` (Warm Concrete) ou `#ffffff` (Surface White).
  * Textos: `#1d1d1f` (True Charcoal) para títulos com peso `font-extrabold` ou `font-extralight` e tracking colado; `#86868b` (Muted Metallic) para textos secundários.
  * Bordas: `rgba(29, 29, 31, 0.06)` (contornos translúcidos ultra-finos).
  * Semipoluição de cores eliminada. Erradicação total de azuis, pretos OLED ou gradientes neon.
* **Efeitos Avançados:**
  * **Liquid Glass (`.apple-glass-card`):** Fundo semi-translúcido com `backdrop-blur(20px)` e contornos `rgba(29,29,31,0.06)`.
  ...
  * **Magnetismo Táctil (`data-magnetic`):** Botões HIG touch targets (≥ 44 × 44px) com atração física e interpolação fluida pelo GSAP.
...
1. **Red Phase:** Escreva um teste direcionado para a rota ou funcionalidade no arquivo de testes adequado. Rode a suíte e veja falhar.
2. **Green Phase:** Implemente a lógica necessária no schema (`src/db/schemas/`) ou controller. Faça o teste passar.
...
* **Prepared Statements Compilados:** Use a API do Drizzle para consultas dinâmicas de alta frequência, melhorando performance e blindando contra injeções SQL.
...
1. **`/gsd-discuss-phase`:** Fase dedicada ao alinhamento técnico e conceitual. Nenhuma alteração produtiva de arquivos é feita.
...
- [ ] O build compilou perfeitamente via `pnpm run build`?
- [ ] O design segue fielmente o Warm Concrete Editorial com cantos Squircle G2 (`rounded-[32px]`)?
